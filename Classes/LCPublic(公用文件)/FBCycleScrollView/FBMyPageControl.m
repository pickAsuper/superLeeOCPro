...
@end
@implementation FBMyPageControl
...
- (id)initWithFrame:(CGRect)frame normalImage:(UIImage *)nImage highlightedImage:(UIImage *)hImage dotsNumber:(NSInteger)pageNum sideLength:(NSInteger)size dotsGap:(NSInteger)gap
{
    ...
    //    self.backgroundColor = [UIColor redColor];
        ...
        for (int i = 0; i != __pageNumbers; ++ i) {
            ...
            dotsImageView.frame = CGRectMake(((size + gap) - 4) * i, 0, size, size);
            dotsImageView.tag = 100 + i;
            if (i == 0) {
                
                self.highlightedDotImageView.frame = CGRectMake(((size + gap)- 6) * i, 0, size, size);
            }
            //            DLog(@"self.highlightedDotImageView = %@",self.highlightedDotImageView);
...
- (void)setCurrentPage:(NSInteger)pages
{
    ...
    if (_normalDotImage || _highlightedDotImage) {
       ...
       // NSLog(@"%f",self.dotsSize + self.dotsGap);
        CGRect newRect = CGRectMake(((self.dotsSize + self.dotsGap)- 4) * pages, 0, self.dotsSize, self.dotsSize);
        
        // CGPoint newOrignal = CGPointMake((self.dotsSize + self.dotsGap) * pages, 0);
      
        [UIView animateWithDuration:0 delay:0.0 options:UIViewAnimationOptionCurveEaseIn animations:^(void){
...
{
    _normalDotImage = nil;
    _highlightedDotImage = nil;
    __dotsArray = nil;
    _highlightedDotImageView = nil;
}
